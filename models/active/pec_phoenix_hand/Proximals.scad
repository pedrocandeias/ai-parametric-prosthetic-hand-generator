...
body_width_front  = 10.6;                // X width at the +Y end (body narrows toward +Y)
...
belly_radius      = 6.4;                 // cylinder radius that rounds the X-Z bottom corners
belly_center_height = 5.0;               // its Z center (keel half-width = sqrt(belly_radius^2 - belly_center_height^2))
...
// underside: each end is a concave cylinder scoop (cylinder axis along X) so the
// body keeps a central keel and its undersides sweep up curved toward each knuckle
scoop_radius   = 13.7;   // scoop cylinder radius
scoop_center_y = 17.4;   // its Y center, +- (large -> the scoop sweeps up gently)
scoop_center_z = 2.8;    // its Z center
...
module body() {
    difference() {
        ...
        underside_scoop( scoop_center_y);   // +Y underside (curved)
        underside_scoop(-scoop_center_y);   // -Y underside (curved)
...
// Concave cylinder scoop carved from one end's underside (cylinder axis along X),
// giving the body a curved underside that sweeps up toward that knuckle.
module underside_scoop(center_y) {
    translate([0, center_y, scoop_center_z]) rotate([0, 90, 0])
        cylinder(r = scoop_radius, h = body_width*$wscale + 4, center = true);
}
...
// The belly rounding: a circle that rounds the X-Z bottom corners (leaving a
// flat keel at Z0), plus a full box above its center so the upper body stays full.
module belly_round() {
    translate([0, body_center_y, belly_center_height])
        rotate([90, 0, 0]) cylinder(r = belly_radius * $wscale, h = body_length + 2, center = true);
    translate([0, body_center_y, belly_center_height + body_raw_height/2])
        cube([body_width*$wscale + 2, body_length + 2, body_raw_height], center = true);
}

// The body block: a flat-bottomed prism that tapers from body_width_rear (-Y)
// to body_width_front (+Y), full height before the top is rounded off.
