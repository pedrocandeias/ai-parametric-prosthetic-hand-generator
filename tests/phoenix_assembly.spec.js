...
test('Phoenix: opens on Assembly, renders seated hand with deps, stays parametric', async ({ page }) => {
    const errors = [];
    ...
    await page.waitForSelector('#param-part', { state: 'attached', timeout: 15000 });

    // 1) Default view must be the assembled hand.
    expect(await page.locator('#param-part').inputValue()).toBe('Assembly');
    expect(await page.locator('#editor').inputValue()).toMatch(/part\s*=\s*"Assembly"/);

    // 2) Render the seated hand (Left, default 82 mm).
    await renderAndWait(page);
    await page.locator('#viewer').screenshot({ path: path.join(SHOT_DIR, '01-assembly-left-82.png') });

    // No OpenSCAD include/use resolution failures for the reconstructed deps.
    ...
    // 3) Parametric: bigger palm + right hand → re-render.
    ...
    await renderAndWait(page);
    await page.locator('#viewer').screenshot({ path: path.join(SHOT_DIR, '02-assembly-right-120.png') });

    // 4) Individual print part still works (Palm), size restored.
    await setControl(page, 'param-LeftRight', 'Left', true);
    await setControl(page, 'param-palm_breadth_mm', '82');
    await setControl(page, 'param-part', 'Palm', true);
    await expect(page.locator('#editor')).toHaveValue(/part\s*=\s*"Palm"/, { timeout: 5000 });
    await renderAndWait(page);
    await page.locator('#viewer').screenshot({ path: path.join(SHOT_DIR, '03-part-palm.png') });

    await page.screenshot({ path: path.join(SHOT_DIR, '04-fullpage.png'), fullPage: true });
