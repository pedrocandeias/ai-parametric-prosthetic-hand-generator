...
fix: **Cyborg Beast forearm gauntlet now shows in the browser.** The model's `cyborg_beast.scad` `include <gauntlet.scad>`s a new self-contained cuff file, but `gauntlet.scad` was never registered in the model's `dependencies`, so the in-browser WASM loader didn't mount it — the `include` resolved locally (renders fine headless) but failed in the app, hiding the gauntlet. Added `gauntlet.scad` to the cyborg_beast `dependencies` so the loader mounts it.
feat: **Expose the gauntlet as parametric, patient-fittable hardware.** Added config parameters + EN/PT captions/help for `show_gauntlet` (toggle), `wrist_circumference_mm` (Anthropometric — sizes the cuff girth to circumference/π + clearance, independently of hand scale), `gauntlet_tilt`, `gauntlet_length_scale`, `wrist_pin_dia`, `wrist_pin_clearance`, `gauntlet_rim_hole_d`, and `color_gauntlet`; every parameter name matches an existing `.scad` variable. Added a **Forearm gauntlet** printable part (`show_gauntlet` toggle) for isolated STL export. Wired `wrist_circumference_mm → wrist.circumference_mm` in `profileMapping.js` (already in the importer schema, previously unplumbed) and noted it in the model's `aiGuidance`, so an imported profile / AI baseline fits the cuff to the patient's wrist. Verified: gauntlet renders seated at the wrist, and the cuff girth scales end-to-end via the parameter (wrist 120 mm → 45.8 mm cuff, 230 mm → 82.1 mm). Bump `index.html` JS cache-buster to 14.38.0.
