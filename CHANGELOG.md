...
fix: **deploy — the FTPS mode no longer stages its own scratch.** `deploy.sh ftp` writes its hash manifest and changed-file list beside the staging dir (`deploy.manifest` / `deploy.changed` in the repo root), so every run after the first collected the previous run's scratch and shipped it to production. Both are now in `EXCLUDES`, next to the rest of the deploy machinery.

## v14.78.0 — 2026-07-29

feat: **UnLimbited Phoenix — the gauntlet is previewed thermoformed (as a tube), not flat.** The arm guard prints as a flat plate and is then heat-formed over the Jig, so the assembled view now shows the formed part while the print-bed layout / STL export keeps the flat `Gauntlet_V4()` unchanged. New generator `scripts/phoenix_bend_gauntlet.py` bakes the wrap once into `phoenix_gauntlet_curved.scad` (`Gauntlet_V4_curved()`, registered as a model dependency): the plate is cut by `x = const` planes — edge-deterministic, so the mesh stays watertight for the manifold backend — and each vertex is remapped by arc length onto a half-elliptical forearm section (flat base, arched top), its own thickness becoming an outward normal offset. The section is set by the hinge, not the jig: the square tab holes sit 30 mm of plate from the crown, so the ears come out **vertical — parallel to the palm's own hinge ears** — exactly when that arc equals the section's quarter perimeter, which solves the arch height from its half-width (28 × 6.95 mm at the ear, rounding out to 66 × 26 mm at the elbow). The hinge bosses are stiff, so they do not roll with the shell either: everything past the bend line (`FLAT_ARC = 22`, clear of the whole boss, which spans arc 22.15..38.14) hangs straight down as one **flat, vertical plane**, so the disc stays exactly round and the face the washer meets is planar and square to the hinge axis. The fold runs on a real radius (`R_FOLD = 2`), since folding on a zero radius makes the plate's own thickness overlap itself on the inside of the crease — the slivers that showed up along the ear neck. Along the guard the bend line *migrates* outward until it reaches the base of the arch, where a vertical drop is what the section does anyway, so the fold fades with no pleating (z cutting planes over the whole length, since the fold's position varies along it; blending the two positions instead crumples the shell, pulling the free edge millimetres out of place). Finally, the ears have to end up **outboard** of the palm's wrist wall (palm boss | ear | washer | pin head) and no amount of bending gets them there: nothing 30 mm along the plate can sit more than 30 mm from the guard's axis, the palm's wall is already 30.92 mm out, and turning the ear 90° spends several mm more. So the whole guard is **stretched sideways** by one uniform factor (~1.49), solved to land the ear's inner faces exactly on the palm's wall at world x 24.08/85.92 — one global scale rather than a local flare, which is what keeps the discs round and the neck free of folds. Their holes sit on the palm's wrist-pin axis at z 8.50; the wrist mouth comes out 61.8 mm wide, matching the palm shell it clamps, opening to ~68 mm at the elbow. Baked rather than transformed live because the coarse plate needs subdividing before any bend, which would cost seconds per render in the browser WASM build. `GAUNT_CURVED = false` in `phoenix_assembly.scad` restores the flat preview.

fix: **UnLimbited Phoenix — tensioner and washers follow the formed guard.** The formed gauntlet's crown sits over the palm shell's instead of on the print plane, so the guard, the tensioner block and the tensioner pins all lift by the same `GAUNT_LIFT` (18.33 mm — the hinge boss hangs a fixed drop below the crown, so that height is what sets it on the palm's wrist-pin axis) — the block would otherwise be swallowed inside the tube instead of riding its dovetail rail — and the guard butts against the wrist seam (y −236.5) with 0.5 mm clearance. The wrist washers are the hinge's spacers, so formed they move outboard onto the ears' outer faces (x 18.41/91.59) instead of sitting against the palm; flat, they stay where they were. Known approximation: the ears are parallel to the palm's, clamped on its wall and with their holes on its pin axis, but still ~13 mm apart along the forearm — closing that would drive the guard's wrist end into the palm shell.
...
feat: **UnLimbited Phoenix — washers get their own visibility toggle** (`show_washers`). The two wrist-pin cup washers are no longer bundled under `show_tensioner`: new `show_washers` boolean in `UnLimbitedPhoenix.scad` / `phoenix_assembly.scad` / `_assembly.scad` and a new Visibility checkbox in `models-config.json` (EN/PT captions + help; `show_tensioner` texts updated to block-and-pins only). The washers also now appear in the print-bed layout (two flat washers beside the tensioner parts) so the toggle exports them like any other part.

fix: **UnLimbited Phoenix — washers seated on the gauntlet wrist holes.** The washers previously floated at the palm wrist pins with their bores along X (both cup faces pointing the same way). The gauntlet previews flat (it thermoforms around the arm), so its wrist holes run through Z: each washer now lies flat on its gauntlet boss, bore concentric with the measured hole centres (31.86,−151)/(92.14,−151) at boss top z=5.8, flat base against the plate and cup up to receive the pin head. `WASHER_ROT`/`WASHER_POS` are now per-washer arrays. Verified by orthographic renders (bores see-through concentric on both sides).

docs: **add palm/hand-body dimension table across the growth series** (`docs/print-validation/palm-body-dimensions.md`). Axis-aligned bounding box (X/Y/Z, mm) of the **hand body = isolated palm** for the three active models (Flexy Beast, Paraglider Hand, UnLimbited Phoenix) at four AI-sized ages (child_8 / teen_15 / adult_28 / elderly_70). The palm already includes the **thumb-mount saddle** (the structure rigidly joined to the palm where the thumb articulates); the thumb digit, the four fingers, and the gauntlet are excluded because they print as separate parts — and because the assembled thumb sits in opposition (pointing −Z), including it would nearly double the measured Z (e.g. Flexy child_8: 37.1 mm palm-only vs 74.5 mm with the posed thumb). Rendered from the stored `params.json` suggestions via OpenSCAD 2026.04.26 in assembled pose (`print_layout=false`) with the palm isolated by toggles; bbox measured with `test/thesis/helpers/stlMetrics.js` (`analyzeSTL`). Three tables: **nominal (digital)** plus **as-printed** dimensions corrected for **natural thermal shrinkage** with **per-part variation** — each part draws its own `s` from a narrow material band (PLA 0.27–0.33 %, PETG 0.37–0.43 %) via a deterministic FNV-1a hash of its identity (model + age + material), so the tables reflect real part-to-part dispersion yet regenerate byte-identically. At the mid-band 0.3 % the PLA figures reproduce the original hand-measured reference table to <0.001 mm, confirming those were as-printed PLA values. Fully reproducible.

## v14.73.0 — 2026-07-14

docs: **add Cyborg Beast teen_15 print project** (`docs/print-validation/cyborg_beast_teen_15_print_project.3mf`). Same live pipeline and same patient text as the Phoenix teen_15 project (`teenage boy, 15 years old, 60kg, 170cm height, Brazil, slim build, arm length 62cm`), now driven through the Cyborg Beast schema: AI sizing via the reconstructed frontend prompt + `POST /api/ai/suggest` grounding (`grounded: true`, `claude-sonnet-4-6`) returned `palm_breadth_mm: 79` with anatomically-ordered fingers (middle 75 longest, pinky 58 shortest, thumb 67) and the proximal/distal split. The 13 printable parts (palm, 4×base+tip fingers, thumb base+tip, gauntlet, thermoform liner) were rendered flat via OpenSCAD `print_layout=true` (`mirrored=false`, left hand) and arranged by Bambu Studio under the **exact Phoenix PLA profile** (Bambu Lab A1 0.4, `0.24mm Draft`, 2 walls, 15% grid, tree(auto) supports @35°, auto_brim 5mm, PLA Basic). Sliced clean across 3 A1 plates: 175.0 g PLA total, ~13h print time.

## v14.72.0 — 2026-07-14

refactor: **coherent parameter interface across all four models.** Every model's parameters now follow the same canonical group order — `Component → Anthropometric → Segment split → Hardware → Gauntlet → Options → Visibility → Colors → Arm` (groups a model lacks are simply skipped) — so switching models in the UI presents a consistent layout instead of a per-model ordering. Cyborg Beast's singleton `Palm liner` group was merged into `Options`. Config-only reorder — parameter names, values and behaviour are unchanged.
refactor: **normalize the laterality parameter — Phoenix `LeftRight` (enum "Left"/"Right") → `mirrored` (boolean)**, matching Flexy Beast, Cyborg Beast and Paraglider. `UnLimbitedPhoenix.scad` now branches on `!mirrored`; the config entry mirrors the shared `mirrored` enum (Left=false/Right=true, `role:"laterality"`). Laterality stays UI-only and role-based, so the AI/profile logic is untouched. No saved-config migration needed: laterality is never restored from saved configs (it is skipped in `applySuggestions`), so the rename cannot silently flip a saved hand. Verified with left/right renders (thumb + finger order correctly mirror).
refactor: **normalize the view toggle — Paraglider `show_assembled` (true=assembled) → `print_layout` (true=flat)**, matching Cyborg Beast and Phoenix. `paraglider_hand.scad` branches on `print_layout`; the model's `exportLayout` is now `{print_layout:true}`. The toggle is preview-only and export-overridden, so old saved configs are unaffected in any geometric way. Verified: assembled preview and flat print-bed layout both render.
test: sync the thesis Phoenix frozen config (`test/thesis/fixtures/frozen-configs.json`) to the renamed `mirrored` param.
chore: tidy the `_applyExportLayout` comment in `app.js` (dropped the obsolete `show_assembled` example; comment-only, no behaviour change).
docs: bring `docs/relatorio-adaptacao-antropometrica.md` fully current and expand it into the complete per-model technical analysis — inherited state, files, scaling formulas and anthropometric wiring for each of the four models (Flexy Beast, Cyborg Beast, Paraglider Reborn, UnLimbited Phoenix), the `profileMapping.js` bridge, the cross-cutting AI/grounding fixes, the Flexy↔Cyborg gauntlet unification (v14.70), and the interface-coherence/parameter-normalization work (v14.72).

## v14.71.0 — 2026-07-14

test: sync the REP-DET frozen Flexy Beast fixture (`test/thesis/fixtures/frozen-configs.json`, config `REP-DET-001`) with the v14.70.0 gauntlet unification. Dropped the now-removed `gauntlet_width_mm`/`gauntlet_length_mm` params and added the Cyborg-shared cuff defaults (`wrist_circumference_mm: 160`, `gauntlet_tilt: 0`, `gauntlet_length_scale: 0.7`, `gauntlet_rim_hole_d: 2.5`). REP-DET compares each render to the first successful run of the same campaign (self-consistency, `ref = ok[0].metrics`) with no stored golden hash, so no baseline regeneration is required.

## v14.70.0 — 2026-07-14

feat: **Flexy Beast now uses the same forearm gauntlet as the Cyborg Beast.** The two hands share the Cyborg Beast palm and wrist hinge (identical wrist-pin axis Y=-27/Z=5.5 and wrist-fin |X|=26.6), so one cuff fits both. The bespoke primitive half-pipe gauntlet that was inlined in `flexy_beast.scad` (~120 lines of `g_*` modules) is removed and replaced by an `include <gauntlet.scad>` — a copy of the Cyborg Beast's organic-shell reconstruction (`Normal Gauntlet with Tensioner`, embedded polyhedron + parametric hole recut). The seating math is ported verbatim from the Cyborg Beast (prongs nest inside the palm wrist fins, auto-seated on the wrist-pin axis for any girth/length/tilt), wrapped in the hand's uniform `scale([xScaleFactor…])` so the local-frame seat lands on the scaled palm bore.
feat: **Flexy Beast gauntlet is now sized anthropometrically.** The old `gauntlet_width_mm`/`gauntlet_length_mm`/`gauntlet_wall_mm`/`gauntlet_pos_adjust`/`strap_splay_adjust` parameters are replaced by the Cyborg Beast set: `wrist_circumference_mm` (canonical anthropometric field — cuff girth = circumference/π + clearance, so imported profiles and AI grounding now auto-size the cuff via `profileMapping.js`), `gauntlet_tilt`, `gauntlet_length_scale`, `gauntlet_rim_hole_d`. `show_gauntlet`, `wrist_pin_dia`, `wrist_pin_clearance` and `color_gauntlet` are unchanged. `gauntlet.scad` registered as a model dependency so the in-browser WASM loader mounts it; `aiGuidance` updated. Verified with local OpenSCAD renders: assembled view, print layout (cuff exports as its own part), wrist-joint seating (prong pin-hole round and coaxial with the palm hinge), and a child-scale render (palm 62 mm / wrist 130 mm) — all manifold, no errors.

## v14.69.0 — 2026-07-13

feat: **thesis evaluation test suite (`test/thesis/`)** — reproducible, protocol-driven infrastructure that produces dissertation evidence for three families: repetibilidade/reprodutibilidade (`REP-*`), robustez (`ROB-*`) and acessibilidade WCAG 2.2 AA (`ACC-*`). New commands `npm run test:thesis[:repetition|:robustness|:a11y:local|:a11y:public|:ai:live]`. Each campaign writes a fresh, non-overwriting evidence dir under `test-results/thesis-evaluation/AAAA-MM-DD_HH-MM-SS_<campaign>/` with `metadados.json`, structured JSON/CSV results, STL/screenshot/trace artefacts, the Playwright HTML report and a SHA-256 `manifesto_evidencias.csv`. REP-DET renders+exports each frozen config ≥10× through the real OpenSCAD-WASM pipeline and classifies geometric equivalence with a pre-declared tolerance (dependency-free binary/ASCII STL mesh analyser: faces, vertices, bbox, volume, watertight/manifold, degenerate faces); REP-XBR compares chromium/firefox/webkit; REP-AI is split into a MOCKED schema/limits/laterality/recovery suite (never a real AI repetition) and an OPT-IN live dispersion suite gated by `RUN_LIVE_AI_TESTS=1`. ROB-* cover limits/invalid/contradiction/uncovered-population/model-floor/service-failure/render-failure/export-without-geometry, each classified passa/falha controlada/falha não controlada/inconclusivo. ACC-* run `@axe-core/playwright` across auth/dashboard/profile/config/AI/3D states plus an explicit manual-verification checklist; the public campaign is strictly non-destructive and unauthenticated. Local campaigns use an isolated synthetic DB on port 3100; no secrets are written to any artefact.
feat: **`HANDFAB_DB_PATH` env seam in `server/db.js`** — optional override of the SQLite path so automated campaigns run against an isolated synthetic database without touching production data. Backward compatible: unset → `data/app.db` as before; no runtime behaviour changes.
feat: **`LOGIN_RATE_LIMIT_MAX` env override in `server/routes/authRoutes.js`** — optional cap on the login rate limiter so automated campaigns that log in repeatedly from one IP are not throttled. Backward compatible: unset in production → the original 5-per-15-minutes limit is unchanged.
chore: add `@axe-core/playwright` devDependency (4.12.1); ignore `test-results/` (large evaluation artefacts, preserved locally and copied selectively to the dissertation annex).
...
refactor: **pec Phoenix palm — thumb dorsal crown de-creased (the "pregas" are gone).** The crumpled triangular pleats and the curling cornice "sail" lip on the thumb dorsal were the `thumb_dome()` skin's own triangulation: every quad split on a fixed diagonal turned the south/cornice saddle rows into convex ridges. Two PART-2-only edits: (1) `THUMB_DOME_Z` interior nodes Laplacian-smoothed (3 passes, clamped |Δ|≤1.6 mm; silhouette rows/cols and the crisp east-bevel kept exact — no fillet), (2) `thumb_dome()` now splits each roof quad along its lower pair (valley-preferring), so the shared edge is a soft groove instead of a convex crease. Pure re-triangulation + node smoothing: dorsal crown reads as one continuous fall, cornice lip gone. Envelope preserved (dorsal deviation vs ghost mean 0.86→0.90 mm, p95/max marginally better); facets 2524→2510 (thumb) / 104484→104462 (full). Cornice/deck/fin/welds untouched; still ghost-free; full model trimesh-watertight, NoError.

## v14.66.0 — 2026-07-13

refactor: **pec Phoenix palm — thumb dome + shell-junction mesh simplified (−36% facets).** PART 2 face-count reduction with the form preserved: `thumb_junction()` sphere `$fn` 32→12 (chord error ~0.02 mm; the visible weld is the ruled hull, not the sphere tessellation) and the redundant `THUMB_JR` y=−2 station dropped (byte-identical to the y=−7 row) → junction 2644→632 facets (−76%); `thumb_dome()` T stations 21→18 (dropped t=7/10/18, each reconstructible to ≤0.30 mm; all 9 S rows kept — the two-way saddle needs them). Thumb-only STL 3934→2524 facets (−35.8%). Surface deviation vs the previous thumb: mean 0.003 mm, max 0.27 mm — silhouette unchanged. Keyhole/channels/prongs/fin line and the shell+floor weld untouched; still ghost-free; full model trimesh-watertight, NoError.

## v14.65.0 — 2026-07-13

fix: **pec Phoenix palm — cable channels no longer sealed by the dorsal fins; fins re-measured; ring/index ramps de-bulged.** PART 4 `finger_fins()`/`finger_fin_caps()` now `difference()` out the shared `cable_channels()` so a fin can never re-close a channel mouth (ghost-style under-blade routing); the pinky mouth (−29.4,26.5,14.8), which was plugged by the pinky spine bridge (not the fin), gets a new `kn_ch1_trench()` notch (same idiom as the existing ch3/ch4 trenches). Flood-fill: all four finger channel mouths now OPEN to air. Fin table re-measured from the ghost (pinky yf 24.4→25.0, base 13→14.5, depth 5.5→4.3; ring/middle/index depth 5.68→4.35 etc.). PART 3: `kn_ramp()` gained an optional mid control row so the ring/index shell→knuckle ramps fall in two chained crisp hulls instead of one straight facet that overshot the ghost's convex fall (ring y25 +4.12→+3.76, index y26 +2.14→+1.61 mm). Honest limit: the rim-adjacent overshoot on the ring/index side is inherited from the PART 1 shell riding ~2–5 mm above the ghost there — nulling it fully needs a shell-station change (out of scope). Full model trimesh-watertight, OpenSCAD NoError; `_assembly_v2.scad` clean, proximal clip fins still engage.

## v14.64.0 — 2026-07-13

feat: **pec Phoenix palm — thumb mount fully parametric; the model is now 100% ghost-free.** PART 2 rebuilt with the Cyborg Beast / paraglider idioms: `thumb_dome()` (skin over measured `THUMB_DOME_T/S/Z` sections along the 50° prong frame), `thumb_body`/`thumb_web`/`thumb_deck`/`thumb_cornice`, parametric `thumb_slot()` clevis pocket, `thumb_fin()` (measured `THUMB_FIN_PTS` on the unchanged blade line), `thumb_trench()` cable saddle, and `thumb_junction()` — the same crown→cowl sphere-hull ramp used at the finger/wrist rims, probed from the current `shell_solid()` (`THUMB_JR_XA/XB`) for a fluid shell weld. The three ghost-clipped pieces (`thumb_fill`/`thumb_shell_junction`/`thumb_top_round`) are gone: `import(ghost_stl)` now appears ONLY in the show_ghost display overlay — the whole palm is parametric. Solid prong overlaps replaced the old tangential graze, so the full model is now trimesh **watertight** (the 2 residual non-manifold edges are fixed) and OpenSCAD NoError. Keyhole bore/slot, cable channels and the fin's functional line unchanged. Known follow-up: the dome arrays are coarse (faceted) and want a density/refinement pass.

## v14.63.0 — 2026-07-12

docs: **validação de impressão — série de crescimento (UnLimbited Phoenix).** Nova campanha em `docs/print-validation/growth-series/`: a mesma pessoa (rapaz brasileiro magro, arquétipo do `teen_15`) aos 8, 12, 15 e 18 anos — altura/peso numa curva ~P50, braço a 0,365 da altura; só a idade varia. Pipeline vivo (prompt do frontend → `/api/ai/suggest` grounded → OpenSCAD `-D` → 3MF), agora sobre o schema Phoenix pós-v14.48: a IA dimensiona os 11 parâmetros (palma + 5 dedos + divisões proximais). 4/4 `grounded: true`; progressão monótona; palma no piso de 82 mm aos 8 e 12 anos **mas com dedos a crescer entre os dois** (42.1→46.1 mm) — piso de escala e comprimento por dedo comprovadamente independentes. Escala verificada na malha (bbox da palma = 82.2/88.2/90.2 mm, rácios exatos 88/82 e 90/82). 28 3MF (+28 PNG): placa combinada `print_layout` + 6 peças isoladas por toggles (`parts/`), assentes em Z=0; README do print-validation atualizado com a secção da série.

## v14.62.0 — 2026-07-12

feat: **pec Phoenix palm — direct inclined transitions front AND back; wrist back wall now parametric (ghost only in the thumb).** Front: new `KN_RAMPS`/`kn_ramp()` — one ruled inclined face per finger band, starting ON the crown skin (`KN_RAMP_Y0`=20.5) and rising over the Y=23 rim clip face to land tangent on the prong crests; the junction slabs became invisible interior fill; rounding defaults dialed down (`KN_EDGE_R` 1.8→0.6, `KN_BULGE`→0) with all tops re-probed on the current shell. Back: PART 6 `wrist_back()` rewritten from a ghost clip into a fully parametric wrist cuff — measured band slabs (`WBAND`, reusing PART 5's `WALL_PROFILE`) + five inclined spans (`WR_RAMPS`/`wr_ramp()`, crown→ears/visor, lip held at z24 to keep the wrist-mouth clearance) with byte-identical ear bore/counterbore re-cuts, shared `cable_channels()` and new strictly-interior `wr_channel_entries()`. The exposed vertical rim faces and recessed channels at both rims are gone. **`import(ghost_stl)` in geometry now exists only in PART 2 (thumb).** Channels improved again (ch1 enclosure 0→100% min); keyholes/windows/fins untouched; ear pin/washer seats verified in `_assembly_v2.scad` with the gauntlet swinging clear.
...
fix: **pec Phoenix palm — thumb mount shape distortion.** Three artifacts diagnosed and fixed in PART 2: the ghost-clipped `thumb_fill()` box bulged through prong B's flat outer face (triangular wedge patches) and its x=38 edge crossed the prong nose (diagonal seam + ledge) — a new `thumb_face_clip()` half-space guard at the prong's outer face plane (`THUMB_YAW`/`THUMB_FACE_U`) extends the flat face cleanly up to the dome; and the old 5×2.5 fin blade + cap primitives (misaligned/short vs the ghost's own fin, so the crown emerged as a chopped fragment and an orphan sliver) were replaced by one generous rounded envelope `thumb_fin()` on the unchanged blade line — the global ghost intersection now keeps the ORIGINAL fin surface as a continuous ridge. Keyhole, channels, prong dims and the fin's functional line untouched (git-diff verified); full model manifold NoError; `_assembly_v2.scad` clean, thumb proximal's clip fin still continues the palm fin.

## v14.59.0 — 2026-07-12

fix: **pec Phoenix assemblies — palm per-part colours actually visible.** In current OpenSCAD an outer `color()` overrides inner ones (verified with a minimal test), so `_assembly.scad`/`_assembly_v2.scad` no longer wrap `model()` in `color(PALM_COL)` — the palm colours itself via the `[Colors]` group.

feat: **pec Phoenix palm — floor tucked under the knuckle/thumb curves (no more 90° steps).** PART 7 front tabs and thumb pads no longer protrude past the curved faces: `FLOOR_FRONT_TABS` is now DERIVED from PART 3's `KN_PRONGS` (auto-tracks the prongs), and new coaxial cutters (`floor_lobe_seat`/`floor_front_cutter`/`floor_thumb_cutter`, the thumb one in the prongs' 50°-yaw frame) carve each tab end into a concave seat that continues the eye-lobe / prong-rounding curve down to the plate. New `[Floor]` params `floor_face_tuck`=0.35, `floor_seat_inset`=0.05; `floor_tab_past` 3.3→4.5 (nominal strip reach; the cut owns the exposed face); the old ghost-derived `FLOOR_FL_PATCH`/`FLOOR_FR_WEDGE` were also tucked under the rebuilt junction slabs. Numeric probe: floor stays 0.07–0.09 mm inside every lobe curve; full model manifold NoError; `_assembly_v2.scad` clean.

## v14.58.0 — 2026-07-12

feat: **pec Phoenix palm — knuckle-band surface smoothing (no more faceted "staircase").** PART 3 refinement round: (1) `kn_rpost()` round-ended cylinder control posts (new `KN_EDGE_R`=1.8) replace the 0.6 mm cube posts, turning every junction hull crease into an arc; (2) `kn_slab()` auto-inserts a bulged mid station (`KN_BULGE`=0.9) on descents >4.5 mm so long flanks are barrels, not flat ruled facets; (3) the rim seam is now near-tangent — `KN_RIM_Y`=22.0 puts the post centres 1 mm behind the rim and every rim-post top was re-probed from the CURRENT `KNEE_R`-filleted shell (the stale pre-fillet profile was the dark recessed band under the crown). Channels improved again (ch1 min-enclosure 67→100%, ch4 avg 83→97%, mouths flood-fill open); keyholes untouched; full model + `_assembly_v2.scad` render NoError.

## v14.57.0 — 2026-07-12

feat: **pec Phoenix palm — per-part colours by default + Customizer reorder.** The `[Features]` show/hide checkboxes moved to the top of `Palm_left_V2.scad`, followed by a new `[Colors]` group with one editable colour per part (shell SkyBlue, knuckles Orange, fins LimeGreen, thumb Tomato, wrist Plum, wrist back MediumPurple, floor Teal — no longer yellow/khaki like its neighbours). `model()` now applies the part colours itself, the solid-yellow default and the separate `model_colored()` are gone, and `debug_colors` now means "explode the thumb into sub-part colours". (Note: in current OpenSCAD an OUTER `color()` overrides inner ones, so the assemblies' `color(PALM_COL)` wrap had to be dropped — see v14.59.0.)

## v14.56.0 — 2026-07-12

feat: **pec Phoenix palm — knuckles (PART 3) fully parametric; shell knee fillet.** The four finger clevises no longer use the ghost STL: `clip_ghost()` deleted; eye lobes are per-physical-prong measured hulls (exact r5.9 foot discs kept, measured hood tops replace the overfill+clip), the backboxes became measured neck/curtain slabs, and the clipped junction band became gap/flank slabs + rounded window pockets whose rim posts follow the parametric shell rim (C0 seam, staircase at the pinky corner removed). Keyholes byte-exact and cut last; shared `cable_channels()` still re-cut (channel enclosure improved: ch1 96%, ch2 100%, mouths flood-fill-verified open). Ghost imports in geometry now: thumb 3 + wrist_back 1 only. Honest note: knuckle-band voxel IoU vs ghost 0.909 → 0.730 — the extra volume is the deliberate smooth shell-continuous junction chosen over raw ghost fidelity (user steer). Also new `KNEE_R` (default 3.5) in `[Shell]`: a per-station fillet cylinder pair tangent to the wall planes rounds the ~90° wall→crown crease, with a concentric cavity copy keeping the wall uniform. Full model manifold NoError; `_assembly_v2.scad` renders clean.

## v14.55.0 — 2026-07-11

feat: **pec Phoenix — `_assembly_v2.scad`, the assembled hand on the IoU-reworked phalanges.** Copy of `_assembly.scad` that `use`s `Proximals_v2.scad` + `Distals_v3.scad` (same seats, tunables and STEP hardware); `_assembly.scad` stays on the original phalanges for comparison. Verified: full and close-up renders clean (0 errors/warnings), fingers/thumb seat correctly with fins and snap pins in the clevises.

## v14.54.0 — 2026-07-11

feat: **pec Phoenix — paraglider-style self-supporting cable channels in the palm.** `Palm_left_V2.scad` channels are now cut by ONE shared `cable_channels()` module (dorsal_shell / front_assembly / wrist_back all use it, so the re-cuts line up) sweeping a "D" cross-section — semicircular floor + chamfered near-flat ceiling, the flexible-flyer channel technique flipped for our floor-down print orientation — with `round_corners` bend smoothing and waypoints retuned to the parametric crown; printed pipe sheaths (`CH_SHEATH_WALL`) carry each channel where it dives out of the roof band. New `[Shell]` params `CH_STYLE` (D / legacy round), `CH_BEND_R`, `CH_SHEATH_WALL`. Verified: sections show all 5 bores embedded with the D ceiling up; shell STL manifold NoError.

feat: **pec Phoenix fingers — fingerator-idiom IoU rework (on copies; originals preserved).** `Proximals_v2.scad`: IoU 0.949 → 0.965 (voxel 0.4 mm; volume ratio 1.002) via 3-station plan taper, asymmetric tilted-ellipse underside scoops, measured 2-circle belly loft and near-planar scoop foot planes; regressions tried and reverted are documented; protected modules (pin_hole, rails, knuckles, fins+caps, pockets) byte-identical (md5-checked). `Distals_v3.scad`: IoU std 0.906 → 0.964, short 0.908 → 0.962, thumb 0.879 → 0.972 (volume ratios 0.992/0.989/0.997) via least-squares-fitted superellipse "egg" sections (fixed 96-vertex skin correspondence), measured pin-concentric slot-end cylinder (r 7.39) + bore-entry counterbores, 5–6 measured tip stations, sloped tendon-groove floor + cord-exit ramp; slot/pin/feat_*/fin invariants byte-identical.

docs: **thesis report — parametric reconstruction of the pec Phoenix hand.** New `docs/pec-phoenix-hand/relatorio-reconstrucao-parametrica.md` (academic European Portuguese, 7 sections, 10 verification figures under `docs/pec-phoenix-hand/img/`): methodology (measure-never-guess, ghost, voxel IoU, manifold), single-file 7-part architecture, the adopted parametric techniques with provenance (Cyborg Beast hull-of-primitives shell, anchored-primitives floor, paraglider D channels + interface inverse-scaling, fingerator phalanx idioms), results and limitations.

## v14.53.0 — 2026-07-11

feat: **pec Phoenix palm — dorsal shell + palm floor rebuilt fully parametric (Cyborg Beast hull technique).** PART 1 `dorsal_shell()` dropped the measured `AY`/`AP` point-cloud loft (~1000 coordinates + BOSL2 `skin()`) for the `cyborgpalm001.scad` hull-of-primitives idiom: one `hull()` over three named control stations (flattened crown ellipsoid + two wall posts each), an independently shrunk cavity hull, then the functional cable-channel cuts. New `[Shell]` Customizer group (`SHELL_WALL`, `SHELL_KNEE_Z`, `CROWN_FLAT/RZ/RY` + per-station `Y/XL/XR/APEX`), defaults = measured envelope. PART 7 `palm_floor_grid()` dropped the measured 45-point outline and its `import(ghost_stl)` front-band clip for a union of named footprint primitives anchored to the parts it mates with (shell feet, clevis aprons + front tabs, ear tabs, thumb pads) smoothed by an offset fillet/round chain; new `[Floor]` Customizer group, basket-weave vents unchanged. Both verified against the ghost overlay and in `_assembly.scad`; full model renders manifold (NoError).

## v14.52.0 — 2026-07-11

feat: **hand side (left/right) is now the first option on every model.** Moved the `role:"laterality"` parameter (`mirrored` / `LeftRight`) to the top of the parameter list in `models/models-config.json` for Cyborg Beast, Flexy Beast, UnLimbited Phoenix and Paraglider Hand Reborn — choosing which hand you are building comes before any measurement. On the Paraglider the selector joins the `Component` group so it sits above the hand/arm component chooser without reordering the groups; everywhere else it leads the `Anthropometric` group. Config-only reorder — parameter names, values and the AI laterality handling (role-based) are untouched.
...
feat: **UnLimbited Phoenix — per-part controls replace the single-part selector.** Dropped the `part` dropdown; every part now has its own always-visible toggle (`show_palm`, `show_fingers`, `show_thumb`, `show_pins`, `show_gauntlet`, `show_tensioner`) plus a `print_layout` switch (assembled seated preview ↔ flat print-bed layout for STL export), matching the Cyborg Beast / Flexy Beast pattern (`exportLayout: {print_layout:true}`, Visibility group). The assembled view and the print layout are both gated by the toggles, scaled uniformly by HandPerc and mirrored for a right hand. Full EN/PT captions + help. Verified end-to-end via Playwright (`tests/phoenix_assembly.spec.js`): the part selector is gone, all six toggles + print_layout render as checkboxes, hiding parts re-renders correctly, the print layout lays parts flat, and it stays parametric (right hand, 100–146%).
fix: **Phoenix preview meshes keep the original palm/fingertip detail.** Replaced the v14.42.0 voxel remesh (manifold but soft/blobby) with a MeshFix repair of the stock non-manifold meshes — it resolves only the self-intersecting spots, preserving the original geometry (palm and fingertips deviate ~0 mm at the 95th percentile) while staying watertight-manifold so the browser's manifold backend renders them in the assembled view. Also smaller (`phoenix_preview_meshes.scad` 623 KB vs 1.9 MB). Bump `index.html` JS cache-buster to 14.45.0.

## v14.44.0 — 2026-07-10

fix: **Cyborg Beast thumb segments now match the original reference STLs.** Measuring the original `Thumb_Phal.stl` (31.2 mm) / `Thumb_Finger_w_Bumps.stl` (36.5 mm) against our SCAD parts showed the reconstructed thumb base was ~6 mm too short (24.8 mm — modelled as a shrunk `fingermid(-12)`). Raised the thumb's native reach reference `THUMB_BASE_REACH` 44.93 → 51.3 mm and set the default `thumb_base_length_mm` 22 → 27, so the native base now measures 31.1 mm and the tip 36.5 mm — both within 0.1 mm of the originals — while the world thumb length stays `thumb_length_mm` (thumb_scale compensates) and the PIP hinge stays coaxial. Verified by isolated-module bbox measurement and an assembled render. (Reference STLs added under `models/active/cyborg-beast/stls/`. The palm-dorsal-thickness discrepancy, ~7 mm, is left for a later pass.) Bump `index.html` JS cache-buster to 14.44.0.

## v14.43.0 — 2026-07-10

feat: **Cyborg Beast palm gains the Flexy Beast thermoformable liner.** Ported the `thermoform_mesh` module and added `show_thermoform` + `color_thermoform`: a perforated mesh (offset grid of elongated holes) clipped to the palm interior (`cyborgbeast07palminsidespace()`, already present in `cyborgpalm001.scad`) lines the inside of the palm so a fitter can heat-form it to the patient's own palm. Rendered in both the assembled view and the print layout (fused with the palm), plus a **Palm liner** printable part and EN/PT captions/help.
fix: **Gauntlet now seats on the real palm wrist-pin axis and its wings meet the palm wings.** The auto-seat was solving to a wrong wrist-pin reference (local y=1.4, z=3.3); corrected to the actual palm wrist hinge (y=-27, z=5.5 — the `hardwarecutouts` bore and wrist-fin pin holes), so the gauntlet moves down/back and its prong pin-holes land on the palm pin (verified by threading a test pin through both).
feat: **Gauntlet widened and decoupled X/Z so its wings always align to the palm.** The cuff is now scaled anisotropically: width (X, prong span) is locked to the palm so the prongs nest just inside the wrist fins at x=±26.6 regardless of the wrist measurement, while depth (Z, girth) still tracks `wrist_circumference_mm`. This makes the two gauntlet wings meet the palm's two wings — previously the prongs shrank to ±16 mm (well inside the ±26.6 fins) at typical wrist sizes. Updated the `wrist_circumference_mm` caption/help accordingly.
feat: New gauntlet defaults `gauntlet_tilt = 0` (forearm in line with the hand instead of drooping 12°) and `gauntlet_length_scale = 0.7` (shorter forearm), in both the `.scad` and the model config. Verified: assembled + print-layout renders and a watertight manifold export. Bump `index.html` JS cache-buster to 14.43.0.

## v14.42.0 — 2026-07-10

fix: **Phoenix assembled preview — restore surface definition on the palm and fingertips.** The v14.41.0 manifold fix voxel-remeshed those two meshes at a coarse pitch (palm 1.25 mm), which made them render but stripped all surface detail (a blobby/melted look). Regenerated at a fine pitch (palm 0.4 mm, fingers 0.32 mm) then reduced with manifold-preserving decimation (`manifold3d.simplify`, 0.11–0.14 mm tolerance) — ~38k/26k triangles, detail recovered while staying watertight-manifold. Also converted them from binary-STL imports to embedded `polyhedron()` modules (`phoenix_preview_meshes.scad`), matching how the rest of the model stores geometry: one 1.9 MB `.scad` dependency replaces the two ~3 MB STL deps (they are fixed meshes, no longer parametric). Verified: manifold render keeps all 9 part colours incl. the palm (39k faces); Playwright E2E passes. Bump `index.html` JS cache-buster to 14.42.0.

## v14.41.0 — 2026-07-10

fix: **Phoenix assembled preview now shows the palm and fingertips.** The v14.36.0 assembled view rendered correctly headless but lost the palm and the four distal (fingertip) segments in the browser. Cause: the in-browser preview uses the **manifold** backend, which silently drops any mesh that fails `PolySet→Manifold` conversion, and the stock `Phoenix_Thermo_Palm_2` (palm) and `Phoenix_Fingers_Left` (distals) are **non-manifold** (self-intersections — edges shared by >2 faces: 32 and 12 respectively). They survive when printed one part at a time (no boolean op), but the assembly's implicit top-level **union** forces conversion → both were dropped, leaving only the manifold proximals (`Phoenix_Phalanx_Left`). Fix: ship watertight **voxel-remeshed** manifold copies (`phoenix_palm_manifold.stl`, `phoenix_fingers_manifold.stl`) that the assembled preview `import()`s; the individual **printable part** options still use the exact original meshes. Registered both as model `dependencies` so the WASM loader mounts them. Verified: manifold render now carries all 9 part colours including the palm (0 → 43k faces); Playwright E2E (`tests/phoenix_assembly.spec.js`) confirms the seated hand renders in-browser with no dependency errors and stays parametric (right hand, 100–146%). Bump `index.html` JS cache-buster to 14.41.0.
fix: **Cyborg Beast gauntlet seats on the actual wrist hinge.** Corrected the palm wrist-pin axis the cuff tracks to the real hardware (`G_PIN_Y = -27`, `G_PIN_Z = 5.5` — the wrist-hinge bore / wrist-fin pin holes) and set neutral defaults (`gauntlet_tilt = 0`, `gauntlet_length_scale = 0.7`) so the forearm guard sits flush at the wrist instead of floating off-axis and drooping.

## v14.40.0 — 2026-07-10

feat: **Cyborg Beast gains a parametric forearm gauntlet** (Normal Gauntlet with Tensioner reconstruction, new `active/cyborg-beast/gauntlet.scad`). The cuff auto-seats on the palm's wrist-pin axis — its prongs nest inside the palm wrist fins and track the pin for any girth/length/tilt — and is sized to the wearer by `wrist_circumference_mm` (cuff width = circumference/π + clearance), independently of hand scale. Exposed parameters: `show_gauntlet`, `wrist_circumference_mm`, `gauntlet_tilt` (forearm droop about the wrist pin), `gauntlet_length_scale`, `wrist_pin_dia` + `wrist_pin_clearance`, `gauntlet_rim_hole_d`, and a 12th per-part colour `color_gauntlet`. Registered as a model dependency so the in-browser WASM loader mounts it. Verified end-to-end: browser render + combined 3MF export bake the gauntlet colour as its own material (`tests/cyborg_export.spec.js` updated to the 12-colour set).
chore: deploy + gitignore now exclude `models/**/output/` and `models/**/previews/` (openscad-skill render scratch) so preview PNGs/STLs never ship to production or land in git.
chore: bump `index.html` JS cache-buster to 14.40.0.

## v14.39.0 — 2026-07-10

fix: remove the "Toggle grid" button from the viewer toolbar — the CSS grid backdrop it toggled was not visible behind the rendered model, so the button had no useful effect.

## v14.38.0 — 2026-07-10

fix: wire up the viewer toolbar buttons on the edit screen — zoom in/out, pan, fullscreen and toggle grid were rendered but had no handlers. Zoom drives the `<model-viewer>` camera-orbit radius; Pan is a toggle mode where left-drag moves the camera target instead of orbiting (capture-phase pointer handlers suppress the built-in orbit); Fullscreen expands the whole viewer card (toolbar included, so the button can also exit); Toggle grid overlays a CSS grid behind the model. E2E coverage in `tests/viewer_controls.spec.js`.

## v14.37.0 — 2026-07-10

feat: **Cyborg Beast fingers and thumb now split into independently-sized proximal (base) and distal (tip) segments.** Each printed finger is two pieces — `fingermid` (proximal phalanx, MCP→PIP) and `fingertip` (distal, PIP→tip) — that were previously locked to a single shared `len` lever, so the proximal/distal proportion was fixed. New per-digit parameters `index/middle/ring/pinky/thumb_base_length_mm` set the proximal segment length directly (MCP→PIP; thumb MCP→IP); the distal segment fills the remainder so the total finger length stays fixed. The proximal span is analytic (`fingermid` hinge-to-hinge = `23 + 2·lp/3`, local units) and the distal lever is solved from the existing measured reach curve (`R = 60.85 + slope·len`), so the two levers reduce exactly to the original shared-`len` finger when the split is left at its native default — defaults render identically to before.
feat: `place_finger()`/`fingerlayout()` refactored to two levers `(lp, ld)`: the knuckle seat tracks the proximal lever only (`9.5 + lp/3`) and the tip is pushed out by `lp/3 + ld/3` so the PIP pin stays coaxial through both segments for any split. Verified by render that the MCP hole stays seated on the palm knuckle line and the PIP hole stays a single clean bore, at the default and at extreme proximal-long / tip-short splits.
feat: **Thumb** gets the same control despite its uniform-scale sizing: `thumb_base_length_mm` sets the proximal fraction, the distal lever holds the sub-assembly reach constant so `thumb_scale()` (and therefore the total thumb length) is unchanged by the split, and the PIP join offset is render-calibrated (`-(23.667 + lp/3 + ld/3)`, reproducing the native `-18` at the `-12/-5` split). Thumb reach model `reach_local = 55.84 + 0.6185·lp + 0.77·ld` fitted on a 5×4 lp/ld render grid (max residual 0.74 mm). Proximal and total both land within a hair of exact (e.g. 22 mm / 38 mm proximal on a 65 mm thumb → 65.0 mm total preserved).
feat: **Anthropometric wiring** — the new `*_base_length_mm` params are mapped in `profileMapping.js` to `digits.<finger>.proximal_length_mm` (already present in the importer schema but previously not plumbed to any model), so an imported patient profile / AI grounding baseline now populates the proximal phalanx split, not just total finger length. Updated the model's `aiGuidance` so the AI knows to set `base_length` only when the proximal phalanx is known (≈0.45 × total finger) and always below the total. Full EN/PT captions + help for all five parameters (`Segment split` group). Bump `index.html` JS cache-buster to 14.37.0.
